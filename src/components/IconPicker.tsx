...
import { iconSetPrettyNames } from './icons/all-sets';
...
const iconsBySetAtom = atom<IconMetadata | null>(null);
const ALL_SETS = '##ALL_SETS##';
const ALLOWED_SETS = ['ion', 'fluent-emoji-flat'];
...
export const IconPicker = ({ data, close }: IconPickerProps) => {
    ...
    const iconsList = useMemo(() => {
        ...
        if (selectedFamily === ALL_SETS) {
            base = [...Object.entries(iconsBySet).filter(([key]) => ALLOWED_SETS.includes(key)), ['custom', customIcons.map(i => i.name)]];
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const json: IconMetadata = await resp.json();
            setIconsBySet(Object.fromEntries(Object.entries(json).filter(([key]) => ALLOWED_SETS.includes(key))));
        };
        if (iconsBySet === null) {
            load();
        }
    ...
    return (<IconPickerContext.Provider value={{ rowRefs, moveFocus }} >
        <div className='IconPicker'>
            ...
            <section className='icons-container'>
                ...
                {iconsBySet === null ? <div>{t('loading')}</div> : <FixedSizeList
